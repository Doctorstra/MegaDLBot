...
async def help(bot, message, cb=False):
    ...
    button = [[
        ...
        InlineKeyboardButton(f'📦 SOURCE', url='https://github.com/Doctorstra'),
...
async def start(bot, message, cb=False):
    ...
    owner_username = owner.username if owner.username else 'mrkpbots'
    button = [[
...
async def cancel_cb(bot, message):
    ...
    try:
        ...
        await print(e)
        await message.answer(
            "Can't Cancel Right Now! Please Try again 😡",
                show_alert=True
            )
